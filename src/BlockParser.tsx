  ...
  return (
    {
      ...
      delimiter: (
        <DelimiterBlock key={item.id} item={item} config={config?.delimiter} />
      ),
      paragraph: (
        <ParagraphBlock key={item.id} item={item} config={config?.paragraph} />
      ),
      columns: (
        <ColumnsBlock
          key={item.id}
          ...
          blockRendererConfig={config}
        />
      ),
    }[item.type] || (
      <ErrorBlock key={item.id} item={item} config={config?.error} />
    )
  );
};
