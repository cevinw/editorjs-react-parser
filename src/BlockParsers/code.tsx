...
const CodeBlock = ({ item, config }: CodeProps): React.JSX.Element => {
  const currentConfig: CodeConfig = Object.assign(
    {},
    defaultCodeConfig,
    config,
  );
  ...
  return (
    <figure key={item.id} className={currentConfig.classNames?.container}>
      {language && (
        <div className={currentConfig.classNames?.languageInfoBar}>
          ...
          />
          <figcaption className={currentConfig.classNames?.languageInfoBarText}>
