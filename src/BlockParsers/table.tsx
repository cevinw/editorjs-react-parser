...
const defaultTableConfig: TableConfig = {
  classNames: {
    tableHeader:
      "first:border-l-0 text-left pl-2 last:border-r-0 border-b-2 border",
...
const TableBlock = ({ item, config }: TableProps): React.JSX.Element => {
  ...
  const rows = item.data.content.map((rowArray: string[], index: number) => {
    const rowItems = rowArray.map((text: string, index2: number) =>
      item.data.withHeadings && index === 0 ? (
        <th className={currentConfig.classNames.tableHeader} key={index2}>
          {text}
        </th>
      ) : (
        <td className={currentConfig.classNames.tableData} key={index2}>
          {text}
        </td>
      ),
    );
  ...
  return (
    <table className={currentConfig.classNames.table} key={item.id}>
      <tbody>{rows}</tbody>
    </table>
  );
};
