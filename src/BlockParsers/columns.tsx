...
const ColumnsBlock = ({
  ...
  const currentConfig: ColumnsConfig = Object.assign(
    {},
  ...
  const innerBlocks = item.data.cols.map((col: OutputData, index: number) => (
    <div
      key={col.time + index.toString()}
      className={currentConfig.classNames?.innerBlocksContainers}
    >
  ...
  return (
    <section
      key={item.id}
      className={
        currentConfig.classNames?.outerContainer +
        " " +
        {
