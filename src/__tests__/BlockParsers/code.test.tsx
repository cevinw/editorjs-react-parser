...
describe("Code Block tests", () => {
    test("Render code snapshot test", async () => {
        const {container} = render(<CodeBlock item={codeBlockData} languages={[
            {language: "typescript", displayText: "Typescript", logoAlt: "ts-logo", logoSrc: "/code-logos/typescript.svg", shortName: "ts"}
        ]}/>);
        expect(container).toMatchSnapshot();
    })

    test("Render code with other codeStyle", async () => {
        const {container} = render(<CodeBlock item={codeBlockData} codeStyle={idea}/>);
